...
void Bluetooth::send(unsigned char *buffer, unsigned int num_bytes){
  ...
  if (handle == INVALID_HANDLE_VALUE){
    throw Nxt_exception::Nxt_exception("send","Bluetooth", BT_INVALID_COM_PORT);
  }
  ...
  if(!WriteFile(handle, byte, num_bytes, &number_bytes, NULL)!=0){
    free(byte);
    throw Nxt_exception::Nxt_exception("send","Bluetooth", BT_ERROR_WRITING_COM_PORT);
  }
...
void Bluetooth::connect(unsigned int comport){
  ...
  ***************************************************************************/
  ...
  try{
    ...
    this->disconnect();
    throw Nxt_exception::Nxt_exception("connect","Bluetooth", BT_UNABLE_TO_CONNECT);
  }
  return;
}
...
void Bluetooth::receive(unsigned char *buffer, unsigned int length){
  ...
  if(ReadFile(handle, byte, length, &number_bytes, NULL) == 0){
    free(byte);
    throw Nxt_exception::Nxt_exception("receive","Bluetooth", BT_ERROR_READING_COM_PORT);
  }
  //printf("Reveive length %d No of bytes %d\n",length, nrBytes);
  if(number_bytes==0 && length != 0){
    free(byte);
    throw Nxt_exception::Nxt_exception("receive","Bluetooth", BT_NO_REPLY);
  }
  i=(unsigned int) number_bytes;
  //printf("No bytes received:%d\n", i);
  if(number_bytes<length){
    free(byte);
    throw Nxt_exception::Nxt_exception("receive","Bluetooth", BT_NOT_ENOUGH_BYTES);
  }
  i=0;

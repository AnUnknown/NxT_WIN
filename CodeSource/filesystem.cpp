...
void Filesystem::download_file(string pc_name, string nxt_name){
  ...
  if((to =fopen(pc_name.c_str(),"wb"))==NULL){
    throw Nxt_exception::Nxt_exception("download_file","Filesystem", PC_FILE_ERROR);
  }
...
void Filesystem::delete_file(string file_name, bool reply){
  ...
  if(reply){
    ...
    if(answer[4]){
      throw Nxt_exception::Nxt_exception("delete_file","Filesystem", 0x00FF & answer[4]);
    }
  }
}
...
void Filesystem::upload_file(string pc_name, string nxt_name){
  ...
  if((from =fopen(pc_name.c_str(),"rb"))==NULL){
    throw Nxt_exception::Nxt_exception("upload_file","Filesystem", PC_FILE_ERROR);
  }
...
int Filesystem::get_first(string wild_card, Nxt_file *file){
  ...
  if(answer[4]){
     if((unsigned int) answer[4]!=  NXT_FILE_NOT_FOUND){
       throw Nxt_exception::Nxt_exception("get_first","Filesystem", 0x00FF & answer[4]);
     }
     else{
       return 0;
     }
...
int Filesystem::get_next(int handle, Nxt_file *file){
  ...
  if(answer[4]){
    if( (unsigned int) answer[4]!=  NXT_FILE_NOT_FOUND){
       throw Nxt_exception::Nxt_exception("get_next","Filesystem", 0x00FF & answer[4]);
     }
     else{
       return 0;
     }
...
Nxt_file Filesystem::open_read(string file_name){
  ...
  if(answer[4]){
     throw Nxt_exception::Nxt_exception("open_read","Filesystem", 0x00FF & answer[4]);
  }
  file.handle = answer[5];
...
Nxt_file Filesystem::open_append(string file_name){
  ...
  connection->receive(&answer[0],10);
  if(answer[4]){
     throw Nxt_exception::Nxt_exception("open_append","Filesystem", 0x00FF & answer[4]);
  }
  file.handle = answer[5];
  file.size = (0xFF & answer[6]) | ((0xFF & answer[7]) << 8)| ((0xFF & answer[8]) << 16)| ((0xFF & answer[9]) << 24);
  i=0;
...
Nxt_file Filesystem::open_write(string file_name, unsigned int file_size, char type){
  ...
  if(answer[4]){
     throw Nxt_exception::Nxt_exception("open_write","Filesystem", 0x00FF & answer[4]);
  }
  file.handle = answer[5];
  file.size = file_size;
  i=0;
...
void Filesystem::close_file(Nxt_file &file){
  ...
  connection->receive(&answer[0],6);
  if(answer[4]){
     throw Nxt_exception::Nxt_exception("close","Filesystem", 0x00FF & answer[4]);
  }
  file.name[0] = '\0';
  file.handle = 0;
  file.size = 0;
...
unsigned short int Filesystem::read_command(Nxt_file &file, unsigned char *buffer, unsigned short int num_bytes){
  ...
  if(answer[4]){
    throw Nxt_exception::Nxt_exception("read_command","Filesystem", 0x00FF & answer[4]);
  }
  bytes_to_read = (0xFF & answer[6]) | ((0xFF & answer[7]) << 8);
  if(bytes_to_read != (num_bytes)){
    throw Nxt_exception::Nxt_exception("read_command","Filesystem", NXT_UNDEFINED_FILE_ERROR);
  }
  i=0;
  while(i<num_bytes){
    buffer[i]=answer[8+i];
...
unsigned short int Filesystem::write_command(Nxt_file &file, unsigned char *buffer, unsigned short int num_bytes){
  ...
  free(command);
  if(answer[4]){
    throw Nxt_exception::Nxt_exception("write_command","Filesystem", 0x00FF & answer[4]);
  }
  bytes_written = (0xFF & answer[6]) | ((0xFF & answer[7]) << 8);
  if(bytes_written != (num_bytes)){
    throw Nxt_exception::Nxt_exception("write_command","Filesystem", NXT_UNDEFINED_FILE_ERROR);
  }
...
void Filesystem::delete_flash(){
  ...
  if(answer[4]){
     throw Nxt_exception::Nxt_exception("delete_flash","Filesystem", 0x00FF & answer[4]);
  }
  return;
}
